  ...
  return (
    <div style={{ display: "flex", justifyContent: "flex-end" }}>
      <button type="submit" disabled={saving} style={{
        padding: "9px 24px", borderRadius: 8, border: "none", fontSize: 14, fontWeight: 700,
        cursor: saving ? "not-allowed" : "pointer",
        background: saving ? "#52525b" : "#eab308", color: "#000",
      }}>
        {saving ? "Saving…" : label}
      </button>
    </div>
...
function Alert({ msg }: { msg: { ok: boolean; text: string } | null }) {
  if (!msg) return null;
  return (
    <div style={{
      padding: "10px 14px", borderRadius: 8, fontSize: 13, fontWeight: 500,
      background: msg.ok ? "#16a34a20" : "#dc262620",
      color:      msg.ok ? "#4ade80"   : "#f87171",
      border: `1px solid ${msg.ok ? "#4ade8040" : "#f8717140"}`,
    }}>{msg.text}</div>
  );
}
...
export default function MemberSettingsPage() {
  ...
  async function saveHealth(e: React.FormEvent) {
    ...
    try {
      ...
      if (bmi)            body.bmi            = parseFloat(bmi);
      if (goal)           body.goal           = goal;
      if (medicalHistory) body.medicalHistory = medicalHistory;
      ...
      if (!res.ok) throw new Error(await res.text());
      setHealthMsg({ ok: true, text: "Health details updated." });
    } catch (err: any) {
      setHealthMsg({ ok: false, text: err.message || "Failed to update health details." });
    } finally {
  ...
  return (
    <div style={{ padding: "28px 24px", maxWidth: 720 }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: "#71717a", marginTop: 4 }}>Update your personal details and account security</p>
      </div>

      {/* Avatar card */}
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 24, padding: "16px 20px", background: "#18181b", borderRadius: 12, border: "1px solid rgba(255,255,255,.07)" }}>
        <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: "#52525b", marginTop: 2 }}>{user?.email}</div>
        </div>
        {bmi && (
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 20, fontWeight: 700, color: parseFloat(bmi) < 18.5 || parseFloat(bmi) > 25 ? "#f97316" : "#4ade80" }}>{bmi}</div>
            <div style={{ fontSize: 11, color: "#52525b" }}>BMI</div>
          </div>
        )}
      </div>
      ...
      <div style={{ display: "flex", gap: 2, marginBottom: 24, background: "#18181b", borderRadius: 10, padding: 4, width: "fit-content", border: "1px solid rgba(255,255,255,.06)" }}>
        {(["profile", "health", "password"] as Tab[]).map(t => (
      ...
      {tab === "profile" && (
        <form onSubmit={saveProfile}>
          <div style={{ background: "#18181b", border: "1px solid rgba(255,255,255,.07)", borderRadius: 12, padding: "24px 20px", display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <Field label="Emergency Contact" hint="Name and phone number of someone we can call in an emergency">
              <input style={inputStyle} value={emergencyContact} onChange={e => setEmergencyContact(e.target.value)} placeholder="e.g. Rahul Sharma – +91 98765 43210" />
            </Field>

            <Field label="Profile Photo URL" hint="Paste a direct image link (JPEG, PNG, WebP)">
      ...
      {tab === "health" && (
        <form onSubmit={saveHealth}>
          <div style={{ background: "#18181b", border: "1px solid rgba(255,255,255,.07)", borderRadius: 12, padding: "24px 20px", display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
              ...
              <Field label="BMI (auto)">
                <div style={{ ...inputStyle, color: bmi ? (parseFloat(bmi) < 18.5 || parseFloat(bmi) > 25 ? "#f97316" : "#4ade80") : "#52525b", fontWeight: 600, display: "flex", alignItems: "center" }}>
                  {bmi ?? "—"}
                  {bmi && <span style={{ fontSize: 11, color: "#71717a", marginLeft: 8, fontWeight: 400 }}>
            ...
            <Field label="Medical History / Conditions" hint="Helps trainers give safer recommendations">
              <textarea
      ...
      {tab === "password" && (
        <form onSubmit={changePassword}>
          <div style={{ background: "#18181b", border: "1px solid rgba(255,255,255,.07)", borderRadius: 12, padding: "24px 20px", display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <Field label="Current Password">
              <div style={{ position: "relative" }}>
                <input
                  ...
                  required
                />
                <button type="button" onClick={() => setShowPw(!showPw)} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#71717a", fontSize: 12 }}>
                  {showPw ? "Hide" : "Show"}
                </button>
              </div>
            </Field>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 4 }}>
                  {pwStrength.map(({ label, ok }) => (
                    <div key={label} style={{
                      flex: 1, textAlign: "center", padding: "4px 0", borderRadius: 4, fontSize: 11, fontWeight: 600,
                      background:  ok ? "#16a34a20" : "#27272a",
                      color:       ok ? "#4ade80"   : "#52525b",
                      border: `1px solid ${ok ? "#4ade8040" : "transparent"}`,
                    }}>
                      {ok ? "✓ " : ""}{label}
                    </div>
                  ))}
            ...
            {/* Match indicator */}
            {confirmPassword && (
              <div style={{ fontSize: 12, color: newPassword === confirmPassword ? "#4ade80" : "#f87171" }}>
